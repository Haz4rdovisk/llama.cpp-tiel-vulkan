...
// Expert-cache small-batch gate. Default=1 preserves PR #27861 behavior.
static int64_t llama_moe_cache_max_tokens() {
    static const int64_t value = []() {
        const char * env = std::getenv("LLAMA_MOE_CACHE_MAX_TOKENS");
        if (env == nullptr || env[0] == '\0') {
            return int64_t(1);
        }
        const long long parsed = std::atoll(env);
        // DEV safety: only the externally validated MTP window is allowed.
        if (parsed < 1 || parsed > 4) {
            LLAMA_LOG_WARN("LLAMA_MOE_CACHE_MAX_TOKENS=%lld outside safe DEV range [1,4]; using 1\n", parsed);
            return int64_t(1);
        }
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (mcache) {
        // dev_table stores four synchronized residency lanes. Select only the
        // 1/2/4 lanes used by this target/MTP batch; no runtime repeat/copy.
        ggml_tensor * mc_table = ggml_view_4d(ctx0, mcache->dev_table,
                1, mcache->dev_table->ne[1], n_tokens, 1,
                mcache->dev_table->nb[1], mcache->dev_table->nb[2], mcache->dev_table->nb[3], 0);
        mc_slot_ids = ggml_get_rows(ctx0, mc_table, selected_experts);
        if (mcache->route_ids) {
            // Keep packed routes for the end-of-graph update without a capture op.
            mc_slot_ids->buffer = mcache->route_ids->buffer;
            mc_slot_ids->data = mcache->route_ids->data;
            GGML_ASSERT(ggml_nbytes(mc_slot_ids) <= ggml_nbytes(mcache->route_ids));
            GGML_ASSERT(ggml_backend_buffer_init_tensor(mc_slot_ids->buffer, mc_slot_ids) == GGML_STATUS_SUCCESS);
        }
